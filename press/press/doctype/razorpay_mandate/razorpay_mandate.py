...
class RazorpayMandate(Document):
	...
	def before_insert(self):
		# Lock the Team row to prevent race condition when creating mandates
		frappe.db.get_value("Team", self.team, "name", for_update=True)

		# Check for existing active or pending mandate
		existing = frappe.db.get_value(
			"Razorpay Mandate",
			{"team": self.team, "status": ("in", ["Active", "Pending"])},
			["name", "status"],
			as_dict=True,
		)
		if existing:
			if existing.status == "Active":
				frappe.throw(
					"An active UPI Autopay mandate already exists. Please cancel it before creating a new one."
				)
			else:
				frappe.throw(
		...
		customer_id = team.create_razorpay_customer()  # no-op if already exists
	...
	def cancel(self, reason: str | None = None):
		"""Cancel the mandate — revokes the Razorpay token if one exists"""
		if self.token_id and self.razorpay_customer_id:
			try:
				client = get_razorpay_client()
				client.token.cancel(self.razorpay_customer_id, self.token_id)
			except Exception:
				from press.utils import log_error

				log_error(
					"Razorpay Token Cancellation Failed",
					mandate=self.name,
